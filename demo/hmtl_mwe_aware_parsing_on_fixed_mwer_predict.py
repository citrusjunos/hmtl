...
class HMTLPredictor:
    ...
    def create_instance(self, snt_info):
        ...
        instance = self.text_to_instance(tokens, pos_tags, head_tags, head_indices)
    ...
    def text_to_instance(self, tokens: List[Token], pos_tags: List[str],
        ...
        _label_namespace: str = "ontonotes_ner_labels"
        _pos_tag_namespace: str = "pos"
        _head_tag_namespace: str = "head_tags"
        _head_indices_namespace: str = "head_indices"
        ...
        fields["pos_tags"] = SequenceLabelField(
            labels=pos_tags, sequence_field=text_field, label_namespace=_pos_tag_namespace
        )

        fields["head_tags"] = SequenceLabelField(
            labels=head_tags, sequence_field=text_field, label_namespace=_head_tag_namespace
        )

        fields["head_indices"] = SequenceLabelField(
            labels=head_indices, sequence_field=text_field, label_namespace=_head_indices_namespace
        )
    ...
    def inference(self, batch, required_tasks):
        ...
        output_parser = self.inference_parser(embedded_text_input_base, encoded_text_ner, mask, batch)
...
def write_prediction(output, snt_info, f_out):
    ...
    for token_id, form, lemma, upos, xpos, head, deprel in zip(ids, forms, lemmas, upos_seq, xpos_seq, pred_heads, pred_deprels):
        cols = [token_id, form, lemma, upos, xpos, "_", head, deprel, "_","_"]
