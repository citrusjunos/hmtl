...
class HMTLPredictor:
    ...
    def create_instance(self, snt_info):
        ...
        instance = self.text_to_instance(tokens)
    ...
    def inference(self, batch, required_tasks):
        ...
        _, embedded_text_input_base, encoded_text_ner, mask = self.inference_ner(batch)

        encoded_text_parser = self.inference_parser(embedded_text_input_base, encoded_text_ner, mask, batch)

        output_vmwer = self.inference_vmwer(embedded_text_input_base, encoded_text_parser , mask, batch)
        decoding_dict_vmwer = self.decode(task_output=output_vmwer, task_name="vmwer")
        final_output["vmwer"] = decoding_dict_vmwer["tags"]

        return final_output

    def inference_parser(self, embedded_text_input_base, encoded_text_ner, mask, batch):
        sub_model = self.model._tagger_dep_parsing
        embedded_text_input = torch.cat([embedded_text_input_base, encoded_text_ner], dim=-1)
    ...
    def inference_ner(self, batch):
        ...
        return None, embedded_text_input_base, encoded_text_ner, mask

    def inference_vmwer(self, embedded_text_input_base, encoded_text_parser, mask, batch):
        submodel = self.model._tagger_vmwer

        embedded_text_input = torch.cat([embedded_text_input_base, encoded_text_parser], dim=-1)

        encoded_text_vmwer = submodel.encoder(embedded_text_input, mask)

        logits = submodel.tag_projection_layer(encoded_text_vmwer)
...
def write_prediction(output, snt_info, f_out):
    ...
    for token_id, form, lemma, upos, xpos, head, deprel, pred_tag in zip(ids, forms, lemmas, upos_seq, xpos_seq, heads, deprels, pred_tags):
        cols = [token_id, form, lemma, upos, xpos, "_", head, deprel, pred_tag]
