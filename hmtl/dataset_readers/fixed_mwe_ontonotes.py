...
@DatasetReader.register("fixed_mwe")
class FixedMweConllReader(DatasetReader):
    """
    A dataset reader to read BIOUL tags of fixed-mwe spans from an Ontonotes dataset
    """
    ...
    def text_to_instance(self, tokens: List[Token], pos_tags: List[str], bio_tags: List[str]) -> Instance:
        ...
        fields["mwe_flgs"] = ArrayField(array=np.array([flg_mwe]))
