...
from hmtl.training.metrics import AttachmentScores
...
@Model.register("biaffine_parser_pipeline_imple")
class BiaffineDependencyParserForPipeline(Model):
    """
    ...
    def __init__(self,
                 ...
                 cuda_device: int = -1,
                 span_emb_calc_method: str = "attention_by_Lee"
    ) -> None:
        super(BiaffineDependencyParserForPipeline, self).__init__(vocab, regularizer)

        self.text_field_embedder = text_field_embedder
        self.encoder = encoder
        self.cuda_device = cuda_device
        self.span_emb_calc_method = span_emb_calc_method

        input_dim_of_encoder = encoder.get_input_dim()
        encoder_dim = encoder.get_output_dim()

        self.extractor = SelfAttentiveSpanExtractor(input_dim=encoder_dim)
        self.extractor._global_attention._module.weight.data.fill_(0.0)
        self.extractor._global_attention._module.bias.data.fill_(0.0)

        nch=256
        num_of_mwe_pos_ids = 15
        self._mlp_for_creating_keys_for_span_emb = \
            torch.nn.Sequential(torch.nn.Linear(encoder_dim, nch),
                                torch.nn.ReLU(True))

        self._linear_layer_for_inner_product_of_query_and_key = \
            torch.nn.Linear(nch, num_of_mwe_pos_ids)


        if self.span_emb_calc_method == "avg":
            span_emb_dim = encoder_dim
        elif self.span_emb_calc_method == "attention_by_Lee":
            span_emb_dim = encoder_dim * 2 + input_dim_of_encoder

        self._span_emb_dim = span_emb_dim

        self.head_arc_feedforward = arc_feedforward or \
                                        FeedForward(span_emb_dim, 1,
        ...
        self._head_sentinel = torch.nn.Parameter(torch.randn([1, 1, span_emb_dim]))
    ...
    def forward(self,  # type: ignore
                ...
                mwe_mask: torch.IntTensor,
                merged_words: Dict[str, torch.LongTensor],
                merged_pos_tags: torch.LongTensor,
                metadata: List[Dict[str, Any]],
                merged_head_tags: torch.LongTensor = None,
                merged_head_indices: torch.LongTensor = None) -> Dict[str, torch.Tensor]:
        ...
        mwe_mask : ``torch.IntTensor``, required.
            A tensor of shape (batch_size, max_merged_snt_len),
            This is a binary sequence.
            A value of each element is 1
                if a token correponds to an MWE (words-with-spaces),
                otherwise 0.

        merged_pos_tags : ``torch.LongTensor``, required.
            The output of a ``SequenceLabelField`` containing POS tags.
            POS tags are required regardless of whether they are used in the model,
            because they are used to filter the evaluation metric to only consider
            heads of words which are not punctuation.
        group_idx :
            # 各tokenにgroup_idを割り付けたもの. 同一MWEは同一group_idを持つ.
            ## e.g., [0, 1, 2, 2, 3, 4]
            # Shape: (batch_size, seq_length)
        merged_head_tags : torch.LongTensor, optional (default = None)
            A torch tensor representing the sequence of integer gold class labels for the arcs
            in the dependency parse. Has shape ``(batch_size, sequence_length)``.
        merged_head_indices : torch.LongTensor, optional (default = None)
        ...
        if merged_pos_tags is not None and self._pos_tag_embedding is not None:
            embedded_pos_tags = self._pos_tag_embedding(merged_pos_tags)
        ...
        # _, _, encoding_dim = encoded_text.size()
        batch_size, max_merged_snt_len = merged_pos_tags.size()

        #print("----- spans and mwe_mask -----")
        #print(spans)
        #print(mwe_mask)

        span_representations = self.extractor(encoded_text, spans)
        # assert list(span_representations.size()) == [batch_size, max_span_num, encoding_dim]
        ...
        head_sentinel = self._head_sentinel.expand(batch_size, 1, self._span_emb_dim)
        # Concatenate the head sentinel onto the sentence representation.
        encoded_text = torch.cat([head_sentinel, encoded_text], 1)
        mask = torch.cat([mask.new_ones(batch_size, 1), mask], 1)
        if merged_head_indices is not None:
            merged_head_indices = torch.cat([merged_head_indices.new_zeros(batch_size, 1), merged_head_indices], 1)
        if merged_head_tags is not None:
            merged_head_tags = torch.cat([merged_head_tags.new_zeros(batch_size, 1), merged_head_tags], 1)
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            ...
            predicted_heads, predicted_head_tags = self._mst_decode(head_tag_representation,
                                                                    ...
                                                                    attended_arcs,
                                                                    mask)
        if merged_head_indices is not None and merged_head_tags is not None:

            arc_nll, tag_nll = self._construct_loss(head_tag_representation=head_tag_representation,
                                                    child_tag_representation=child_tag_representation,
                                                    attended_arcs=attended_arcs,
                                                    head_indices=merged_head_indices,
                                                    head_tags=merged_head_tags,
                                                    mask=mask)
            loss = arc_nll + tag_nll

            evaluation_mask = self._get_mask_for_eval(mask[:, 1:], merged_pos_tags)
            # We calculate attatchment scores for the whole sentence
            # but excluding the symbolic ROOT token at the start,
            # which is why we start from the second element in the sequence.
            self._attachment_scores(predicted_heads[:, 1:],
                                    predicted_head_tags[:, 1:],
                                    merged_head_indices[:, 1:],
                                    merged_head_tags[:, 1:],
                                    mwe_mask,
                                    evaluation_mask,
                                    )
