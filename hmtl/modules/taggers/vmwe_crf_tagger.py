...
from hmtl.training.metrics import GappySpanBasedF1Measure

@Model.register("vmwe_crf_tagger")
class VmweCrfTagger(Model):
    """
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        if calculate_span_f1:
            self._f1_metric = GappySpanBasedF1Measure(vocab,
    ...
    def forward(self,  # type: ignore
                ...
                vmwe_flgs: torch.LongTensor = None,
        ...
        if tags is not None:
            ...
            positive_nums = int(sum(vmwe_flgs))
            if positive_nums < 1:
            ...
            if negative_nums > 0 and positive_nums < negative_nums:
                ...
                mask_for_negative_instances = vmwe_dropout(mask_for_negative_instances)
                ...
                for flg in vmwe_flgs:
                    if flg==1:
                        ...
                        if val > 0:
                            val = 1 
                        vmwe_mask.append(val)
                        cnt+=1

                effective_batch_size = sum(vmwe_mask)

                batch_size, seq_len = mask.size()
                batch_size, seq_len, num_tags = logits.size()

                vmwe_mask = torch.ByteTensor(vmwe_mask)
                vmwe_mask = vmwe_mask.cuda(self.cuda_device)

                vmwe_mask.unsqueeze_(-1)
                vmwe_mask = vmwe_mask.expand(batch_size, seq_len)

                #print(vmwe_mask)
                mask = torch.masked_select(mask, vmwe_mask).view(effective_batch_size, seq_len)
                tags = torch.masked_select(tags, vmwe_mask).view(effective_batch_size, seq_len)

                vmwe_mask.unsqueeze_(-1)
                vmwe_mask = vmwe_mask.expand(batch_size, seq_len, num_tags)
                logits = torch.masked_select(logits, vmwe_mask).view(effective_batch_size, seq_len, num_tags)

            # Add negative log-likelihood as loss
            #print("--- sizes of logits, tags, mask ")
            #print(logits.size())
            #print(tags.size())
            #print(mask.size())
