...
@Metric.register("attachment_scores_with_mwe")
class AttachmentScores(Metric):
    ...
    def __init__(self, ignore_classes: List[int] = None) -> None:
        ...
        self._mwe_head_labeled_correct = 0.
        self._mwe_head_unlabeled_correct = 0.
        self._total_mwe_heads = 0.
    ...
    def __call__(self, # type: ignore
        ...
        unwrapped = self.unwrap_to_tensors(predicted_indices, predicted_labels,
                                           gold_indices, gold_labels, mwe_mask, mask)
        predicted_indices, predicted_labels, gold_indices, gold_labels, mwe_mask, mask = unwrapped
        ...
        correct_mwe_head_indices = correct_indices * mwe_mask
        correct_mwe_head_labels = correct_labels * mwe_mask
        correct_mwe_head_labels_and_indices = correct_mwe_head_indices * correct_mwe_head_labels
        ...
        self._mwe_head_unlabeled_correct += correct_mwe_head_indices.sum()
        self._mwe_head_labeled_correct += correct_mwe_head_labels_and_indices.sum()
        self._total_mwe_heads += mwe_mask.sum()
    ...
    def get_metric(self, reset: bool = False):
        ...
        mwe_head_unlabeled_attachment_score = 0.0
        mwe_head_labeled_attachment_score = 0.0
        ...
        if self._total_mwe_heads > 0.0:
            mwe_head_unlabeled_attachment_score = float(self._mwe_head_unlabeled_correct) / float(self._total_mwe_heads)
            mwe_head_labeled_attachment_score = float(self._mwe_head_labeled_correct) / float(self._total_mwe_heads)
        ...
        return {
                ...
                "mwe_head_UAS": mwe_head_unlabeled_attachment_score,
                "mwe_head_LAS": mwe_head_labeled_attachment_score,
        }
