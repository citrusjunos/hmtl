...
@Metric.register("gappy_span_f1")
class GappySpanBasedF1Measure(Metric):
    def __init__(self,
        ...
        self.scores = defaultdict(lambda: defaultdict(Counter))
    ...
    def __call__(self,
        ...
        for i in range(batch_size):
            ...
            self.eval_sent_links(gold_string_labels, predicted_string_labels, self.scores)

    def eval_sent_links(self, goldmwetags, predmwetags, counts):
        ...
        if flg_valid_prediction:
            ...
            for j, (goldTag, predTag) in enumerate(zip(goldmwetags, predmwetags)):
                ...
                g_last_BI, g_last_bi, glinks = self.update_info(j, goldTag, g_last_BI, g_last_bi, glinks)
                p_last_BI, p_last_bi, plinks = self.update_info(j, predTag, p_last_BI, p_last_bi, plinks)
            ...
            for d in ('Link+', 'Link-'):  # Link+ = strengthen weak links, Link- = remove weak links
                ...
                if flg_link_based:
                    ...
                    c['PNumer'] += sum(1 for a, b, mwepos in plinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos for grp in ggroups1))
                    c['PDenom'] += len(plinks1)
                    c['RNumer'] += sum(1 for a, b, mwepos in glinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos for grp in pgroups1))
                    c['RDenom'] += len(glinks1)

                    c['PNumerVB'] += sum(1 for a, b, mwepos in plinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos and mwepos == "VB" for grp
                        in ggroups1))
                    c['PDenomVB'] += len([a for a, b, mwepos in plinks1 if mwepos == "VB"])
                    c['RNumerVB'] += sum(1 for a, b, mwepos in glinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos and mwepos == "VB" for grp
                        in pgroups1))
                    c['RDenomVB'] += len([a for a, b, mwepos in glinks1 if mwepos == "VB"])

                    c['PNumerCONT'] += sum(1 for a, b, mwepos in plinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos and mwepos != "VB" for grp
                        in ggroups1))
                    c['PDenomCONT'] += len([a for a, b, mwepos in plinks1 if mwepos != "VB"])
                    c['RNumerCONT'] += sum(1 for a, b, mwepos in glinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos and mwepos != "VB" for grp
                        in pgroups1))
                ...
                if False:
                    ...
                    c['PNumer'] += sum((1 if b - a > 1 else 0) for a, b, mwepos in plinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos for grp in ggroups1))
                    c['PDenom'] += sum((1 if b - a > 1 else 0) for a, b, mwepos in plinks1)
                    c['RNumer'] += sum((1 if b - a > 1 else 0) for a, b, mwepos in glinks1 if any(
                        a in grp["elems"] and b in grp["elems"] and grp["mwepos"] == mwepos for grp in pgroups1))
    ...
    def update_info(self, j, orgTag, last_BI, last_bi, links):
            ...
            if k[1] =='Tags':
                if k[0] in ('MWE'):
                    ...
                    for m in ('P', 'R', 'F', 'P-VB', 'R-VB', 'F-VB', 'P-CONT', 'R-CONT', 'F-CONT'):
                        # strength averaging
                        avg = (self.scores[k]['Link+'][m]+self.scores[k]['Link-'][m])/2   # float
                        # construct a ratio by averaging the denominators (this gives insight into underlying recall-denominators)
                        self.scores[k]['LinkAvg'][m] = avg


        all_metrics = {}
        all_metrics["precision-overall"] = self.scores['MWE','Tags']['LinkAvg']['P']
        all_metrics["recall-overall"] = self.scores['MWE','Tags']['LinkAvg']['R']
        all_metrics["f1-measure-overall"] = self.scores['MWE','Tags']['LinkAvg']['F']

        all_metrics["precision-overall-VB"] = self.scores['MWE','Tags']['LinkAvg']['P-VB']
        all_metrics["recall-overall-VB"] = self.scores['MWE','Tags']['LinkAvg']['R-VB']
        all_metrics["f1-measure-overall-VB"] = self.scores['MWE','Tags']['LinkAvg']['F-VB']

        all_metrics["precision-overall-CONT"] = self.scores['MWE','Tags']['LinkAvg']['P-CONT']
        all_metrics["recall-overall-CONT"] = self.scores['MWE','Tags']['LinkAvg']['R-CONT']
        all_metrics["f1-measure-overall-CONT"] = self.scores['MWE','Tags']['LinkAvg']['F-CONT']
